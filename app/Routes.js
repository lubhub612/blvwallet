...
import Bitcoinloved from './components/Bitcoinloved';
...
export default class RouteApp extends React.Component<Props, AppState> {
  ...
  setBlvPrice = (price: number | null) => {
    console.log(`Price = ${price}`);
    const { info } = this.state;

    const newInfo = new Info();
    Object.assign(newInfo, info);
    newInfo.blvPrice = price;
  ...
  setInfo = (newInfo: Info) => {
    ...
    if (!newInfo.blvPrice) {
      ...
      newInfo.blvPrice = info.blvPrice;
    }
        ...
        <div style={{ overflow: 'hidden' }}>
          ...
          <div className={cstyles.contentcontainer}>
            <Switch>
              ...
              <Route path={routes.BITCOINLOVED} render={() => <Bitcoinloved info={info} refresh={this.doRefresh} />} />
