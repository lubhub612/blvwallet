...
const TxModalInternal = ({ modalIsOpen, tx, closeModal, currencyName, blvPrice, setSendTo, history }) => {
  let txid = '';
  let type = '';
  let typeIcon = '';
  let typeColor = '';
      ...
      <div className={[cstyles.verticalflex].join(' ')}>
        ...
        <div className={[cstyles.center].join(' ')}>
          ...
          <BalanceBlockHighlight
            blvValue={amount}
            usdValue={Utils.getBlvToUsdString(blvPrice, Math.abs(amount))}
        ...
        {detailedTxns.map(txdetail => {
          const { bigPart, smallPart } = Utils.splitBlvAmountIntoBigSmall(Math.abs(txdetail.amount));
...
const TxItemBlock = ({ transaction, currencyName, blvPrice, txClicked, addressBookMap }) => {
              ...
              <div key={address} className={cstyles.padtopsmall}>
                ...
                <div className={[styles.txamount, cstyles.right].join(' ')}>
                  ...
                  <div className={[cstyles.sublight, cstyles.small, cstyles.padtopsmall].join(' ')}>
                    {Utils.getBlvToUsdString(blvPrice, Math.abs(txdetail.amount))}
                  </div>
                </div>
              </div>
            );
