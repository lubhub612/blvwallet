...
export const NO_CONNECTION: string = 'Could not connect to bitcoinloved';
...
export default class Utils {
  ...
  static isTransparent(addr: string): boolean {
    if (!addr) return false;
    return new RegExp('^B[a-zA-Z0-9]{34}$').test(addr);
  }
  ...
  static splitBlvAmountIntoBigSmall(blvValue: number) {
    if (!blvValue) {
      return { bigPart: blvValue, smallPart: '' };
    }

    let bigPart = Utils.maxPrecision(blvValue);
    let smallPart = '';
  ...
  static getDefaultDonationMemo(testnet: boolean): string {
    return 'Thanks for supporting Blvwallet!';
  }

  static getBlvToUsdString(price: number | null, blvValue: number | null): string {
    if (!price || !blvValue) {
      return 'USD --';
    }

    return `USD ${(price * blvValue).toFixed(2)}`;
  }
}
