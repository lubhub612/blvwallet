...
export default class RPC {
  ...
  constructor(
    ...
    this.fnSetBlvPrice = fnSetBlvPrice;
  ...
  async configure(rpcConfig: RPCConfig) {
    ...
    if (!this.priceTimerID) {
      this.priceTimerID = setTimeout(() => this.getBlvPrice(), 1000);
    }
  }
  ...
  static async getInfoObject(rpcConfig: RPCConfig) {
    ...
    info.currencyName = info.testnet ? 'TESTBLV' : 'BLV';
    info.blvPrice = null; // Setting this to null will copy over the existing price
  ...
  setupNextBlvPriceRefresh(retryCount: number, timeout: number) {
    // Every hour
    this.priceTimerID = setTimeout(() => this.getBlvPrice(retryCount), timeout);
  }

  async getBlvPrice(retryCount: number) {
    if (!retryCount) {
    ...
    try {
      const response = await new Promise((resolve, reject) => {
        axios('https://api.coincap.io/v2/rates/bitcoinlove', {
          method: 'GET'
        })
          .then(r => resolve(r.data))
          .catch(err => {
      ...
      const blvData = response.data;
      if (blvData) {
        this.fnSetBlvPrice(blvData.rateUsd);
        this.setupNextBlvPriceRefresh(0, 1000 * 60 * 60); // Every hour
      } else {
        this.fnSetBlvPrice(null);
        let timeout = 1000 * 60; // 1 minute
        if (retryCount > 5) {
          timeout = 1000 * 60 * 60; // an hour later
        }
        this.setupNextBlvPriceRefresh(retryCount + 1, timeout);
      }
    } catch (err) {
      console.log(err);
      this.fnSetBlvPrice(null);
